...
int main()
{
	...
	if (pas == '1') { // Įvedami duomenys
		...
		try {
			...
			laikinas.read(open_f, grupe); // Kreipimasis į skaitymo funkcija
		}
		catch (const char* msg) {
			cerr << msg << endl;
			return EXIT_FAILURE;
		...
		do {
			...
			if (rink == '1') {
				...
				for (int i = 0; i < stud_kiek; i++) {
					laikinas.pazresize(nd_kiekis);
					try {
						laikinas.failai(nd_kiekis, i, out_f);
					}
					catch (const char* msg) {
						cerr << msg << endl;
						terminate();
			...
			if (strat == '1') laikinas.isrinkimas1(grupe, Nel, Kiet); //Pirma strategija 
			else if (strat == '2') laikinas.isrinkimas2(grupe, Nel);  //Antra strategija


			auto start4 = std::chrono::high_resolution_clock::now(); auto st4 = start4; //Pradedama nelaimeliu spausdinimo laikas
			for (auto& i : Nel) i.spausd(pas2, pas3, out_n); // Atspausdinama nelaimeliai
			...
			if (strat == '1') for (auto& i : Kiet) i.spausd(pas2, pas3, out_k); //Atspausdinama kietekai
			else if (strat == '2') for (auto& i : grupe) i.spausd(pas2, pas3, out_k); //Atspausdinama kietekai
	...
	if (pas != '4') {
		...
		if (pas3 == 'n') { // Spausdinama į terminalą
			...
			cout << "--------------------------------------------------------------------------------------------" << endl;
			for (auto& i : grupe) i.spausd(pas2, pas3, out_f);
		}
